...
void BISHOP::computeValidMoves(const POSITION &from, const CHESS &state)
{
    ...
    for (const MOVE &m : rawMoves) {
        ...
        if (color == COLOR::WHITE) {
            // here I'm checking if the white bishop leaves the back rank (rank 1)
            if (from.rank == 1 && m.to.rank > 1) {
                // here I'm marking this bishop move as development
                isDev = true;
            }
        } else {
            // here I'm checking if the black bishop leaves the back rank (rank 8)
