...
void ROOK::computeValidMoves(const POSITION &from, const CHESS &state)
{
    ...
    for (const MOVE &m : rawMoves) {
        ...
        // isDevelopment heuristic
        bool isDev = false;
        if (!isCapture && !givesCheck) {
            // Here I'm checking for rook development:
            if (color == COLOR::WHITE) {
                // Here I'm checking if the white rook moves off its initial back rank (rank 1).
                // If from.rank == 1 and to.rank > 1, it's a developing move.
                if (from.rank == 1 && m.to.rank > 1) {
                    // here I'm marking this rook move as development
                    isDev = true;
                }
            } else {
                // Here I'm checking if the black rook moves off its initial back rank (rank 8).
                // If from.rank == 8 and to.rank < 8, it's a developing move.
