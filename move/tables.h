...
extern const uint16_t MVV_LVA[5][5];
...
extern Bitboard KING_QUADRANT[64];

static inline __attribute__((always_inline))
void build_king_quadrant() {
  for (int sq = 0; sq < 64; ++sq) {
    Bitboard m = 0ULL;
    int rk = sq >> 3;      // 0–7
    int fl = sq & 7;       // 0–7
    // dr,df ∈ {-2,-1,0,+1,+2} gives us the 5×5 block around sq
    for (int dr = -2; dr <= 2; ++dr) {
      for (int df = -2; df <= 2; ++df) {
        int r2 = rk + dr;
        int f2 = fl + df;
        if ((unsigned)r2 < 8u && (unsigned)f2 < 8u) {
          int sq2 = (r2 << 3) | f2;
          m |= (Bitboard(1) << sq2);
        }
      }
